...
/*
 *This method is primarily used for extension A to check whether the correct file mode is allowed 
* UNRESTRICTED=0
* EXCLSUIVE=1
* TRANSACTION=2
*/ 
int isValid(clientPacketData* packet)
{
		pthread_mutex_lock(&userListMutex);

	//printf("ENTER THIS METHOD!\n");
	//printf("What is the packet's permissionMode: %d\n",packet->permissionMode);
	//printf("WHAT IS THE PACKET's flags...........: %d\n",packet->modeFlags);
	
	//This method is called for the first time so the tableFD is NULL 
	//allocate space for the client request with the sizeof being FileDescriptorTable 
	//and then sets its next to value=NULL
	//and set the packetData, which is the metadata from the client requests to be equal to the incoming packet
    	if(tableFD==NULL)
	{
        	tableFD=malloc(sizeof(FileDescriptorTable));
        	tableFD->packetData=packet;
		tableFD->next=NULL;
         	pthread_mutex_unlock(&userListMutex);
		return 1;
    	}
    	...
    	FileDescriptorTable* currentTable=tableFD;
	int areTransaction = 0;
	int areExclusive = 0;
	int areUnrestricted = 0;
	int areExclusiveWrites = 0;
	int areUnrestrictedWrites = 0;

    	while(currentTable!=NULL)
	{
        	if((strcmp(currentTable->packetData->fileName,packet->fileName)==0))
		{
			if(currentTable->packetData->permissionMode == 0)//Unrestricted
			{
				areUnrestricted=1;
				if(currentTable->packetData->modeFlags!=O_RDONLY)
				{
					areUnrestrictedWrites=1;
				}
          		}
			else if(currentTable->packetData->permissionMode==1)
			{
				areExclusive=1;
				if(currentTable->packetData->modeFlags != O_RDONLY)
				{
					areExclusiveWrites=1;
				}
			}
			else if(currentTable->packetData->permissionMode == 2)
			{
				areTransaction = 1;
			}
		}
	...
	if(areTransaction) //this file is open in transation mode
	{
		printf("Client has been denied access since a file has Transaction mode on!\n");
		pthread_mutex_unlock(&userListMutex);
		return DENIED_ACCESS;
	}
	//file is not open in transation mode
	//request is read-only--->depends on permissionMode of request
	if(packet->modeFlags==O_RDONLY)
	{
		//if not transation, reads always allowed
		if(packet->permissionMode!=2)
		{
			//Adding into FileDescriptorTable
		...
		//permissionMode of request is transaction. If the file is open any other mode, disallow
		if(areExclusive||areUnrestricted)
		{
			printf("Client has been denied access since a previous file has Exclusive or Unrestricted mode on!\n");
			pthread_mutex_unlock(&userListMutex);
			return DENIED_ACCESS;
		}
		//request allowed
	...
	//Here, NO request is read-only
	//Now it depends on the permissionMode of the request
	if(packet->permissionMode==0)
	{
		//check if there are any modes in exclusive mode
		if(areExclusive==0)	//there aren't any
		{
			FileDescriptorTable* newPacket=(FileDescriptorTable*)malloc(sizeof(FileDescriptorTable));
			newPacket->packetData=packet;
			insertLinkedList(newPacket);
		...
		if(areExclusiveWrites)//there are, check if someone is writing already
		{
			printf("Client has been denied access since a file is writing with Exclusive mode on!\n");
			pthread_mutex_unlock(&userListMutex);
			return DENIED_ACCESS;
		}
		//Adding into FileDescriptor Table
		FileDescriptorTable* newPacket=(FileDescriptorTable*)malloc(sizeof(FileDescriptorTable));
		newPacket->packetData=packet;
		insertLinkedList(newPacket);
		pthread_mutex_unlock(&userListMutex);
	...
	if(packet->permissionMode==1)
	{
		//check if there are any other write requests
		if(areExclusiveWrites||areUnrestrictedWrites)//there are
		{
			printf("Client has been denied access since a previousFDTableious file is writing with Exclusive mode on!\n");
			pthread_mutex_unlock(&userListMutex);
			return DENIED_ACCESS;
		}
		//Adding into FileDescriptor Table
	...
	if(packet->permissionMode==2)
	{
		//check if the file is open in any other mode
		if(areExclusive||areUnrestricted)
		{
			printf("Client has been denied access since a previousFDTableious file has Exclusive or Unrestricted mode on!\n");
			pthread_mutex_unlock(&userListMutex);
			return DENIED_ACCESS;
		}
		//Adding into FileDescriptor Table
...
clientPacketData* handleOpenRequest(clientPacketData* packet,char buffer[MAXBUFFERSIZE],int errorNumber)
{
	...
	//and we return the negative version of the server file descriptor back to the client side
      	...
      	int exists=0;
	//For extension A to check whether or not their is a file that has more restrictive permission mode
      	if(result!=-1)
      	{
	 	//pthread_mutex_lock(&userListMutex);
         	exists=isValid(packet);
	 	//pthread_mutex_unlock(&userListMutex);
      	}
	//check the result from isValid
      	if(exists!=0&&exists<0)
      	{
		//if -1 then print error message from netopen request
        	result=-1;
        	printf("ERROR: NetOpen request cannot open file,since there is another file open with a more restricted mode\n");
      	}
	...
	//if so then we have a bad file descriptor
      	...
      	if(result==-1)
      	{
		//if isValid method returned -1, then set errorNumber to DENIED_ACCESS
        	if(exists<0)
		{
            		errorNumber=DENIED_ACCESS;
        	}
		else
		{
			//Netopen fails so we send back errno back to the client
...
int main(int argc, char * argv[])
{
  	...
  	while(1)
	{
		...
		//get the metadata from the client side
    		if((modemsg=recv(serverFileDescriptor,&filemode,sizeof(filemode),0))==-1)
		{
			//RECEIVING THE CORRESPONDING FILE MODE
      			perror("ERROR: Issue with receiving the file mode from the user");
    		}
		int mode=0;
		//Initialize fields for client request
    		mode=ntohl(filemode);
    		packet->permissionMode=mode;
