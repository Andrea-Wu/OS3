...
int connectionStatus; //this is determined by the file mode in netserverinit();
...
{
	//convert into 32-bit integer into host byte order
	int modeMessageOpen=htonl(connectionStatus);
	//send client request to the server to handle the actual operations 
        if(send(socketDescriptor,&modeMessageOpen,sizeof(modeMessageOpen),0)==-1)
        {
                perror("Netopen, Unable to send File Mode.");
        }
	//make the thread sleep for 1 second in case of multiple threads incoming
	sleep(1);
...
ssize_t netwrite(int filedes,const void *buf,size_t nbyte)
{
	...
	//printf("netwrite: Sending File Mode.\n");
    	int modeMessage=htonl(connectionStatus);
	
    	if(send(socketDescriptor,&modeMessage,sizeof(modeMessage),0)==-1)
	{
        	perror("ERROR: NetWrite reuqest could not receive the file modes!\n");
    	}
	sleep(1);
...
int netclose(int fd)
{
	...
	int modeMessage=htonl(connectionStatus);
    	if(send(socketDescriptor,&modeMessage,sizeof(modeMessage),0)==-1)
	{
        	perror("ERROR: NetClose request could not send file modes");
    	}
	sleep(1);
